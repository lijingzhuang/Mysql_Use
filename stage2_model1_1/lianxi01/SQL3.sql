...
-- 1.2 增加条件：平均成绩大于70
*/
SELECT id,NAME,AVG(score) FROM student s,student_course sc 
WHERE sc.`score` > (SELECT AVG(score) FROM student_course);
...
SELECT DISTINCT
	...
	COUNT(sc.`course_id`),
	SUM(sc.`score`)
FROM student s,student_course sc WHERE s.`id` = sc.`student_id` GROUP BY s.`NAME`,s.`id`;
/*第三题
...
SELECT 
	t.`id`
FROM teacher t WHERE t.`NAME` = '赵云';
SELECT
	c.`id` ,
	s.`id` AS '学号',
	s.`NAME` AS '姓名'
FROM course c,student s WHERE c.`id` = (SELECT t.`id` FROM teacher t WHERE t.`NAME` = '赵云');
/*第四题
-- 4. 查询选课 少于三门学科的学员			
...
GROUP BY s.`id`,s.`NAME` HAVING COUNT(sc.`course_id`) <= 3;
